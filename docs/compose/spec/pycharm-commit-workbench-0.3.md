...
- 本次修订：文件区与提交区之间支持指针拖动和键盘调节；普通视口至少保留 48px 文件区，紧凑视口至少保留 24px。正常松手、指针取消或捕获丢失均保存最后有效高度，拖动期间的窗口变化不再恢复旧偏好。
...
- 本次修订：AI 加载圆环默认使用原生 `hidden` 属性隐藏，仅在生成期间显示并旋转，完成、失败或取消后恢复隐藏。
...
- 独立审查：布局边界、拖动中断事件顺序、档位可用状态和圆环显示状态均通过复审；未发现阻断问题。
- 安装包：`gitool-file-commit-0.3.0.vsix` 共 15 个文件，不含 `history.js`；最终 SHA-256 为 `bb41c0b248996ba67f8b07915037ac5d016df85e211352b3c42090294109aa10`。
...
- 指针取消与捕获丢失最初会恢复拖动前高度，造成偶发回弹；最终统一为保存最后有效高度，并让拖动期间的 `resize` 只裁剪当前高度。
- 独立复审确认 300px 与 360px 视口拖到最大后仍保留一行文件区，全部阻断项已关闭。
- 用户截图先后确认档位下拉在 `0 / 27` 时被错误禁用、圆环在空闲时常驻；前者通过拆分选择与生成条件修复，后者定位为 Codicon 高优先级 `display` 覆盖隐藏规则，改用原生 `hidden` 契约修复。
...
- [x] T12: 修复拖动回弹 — acceptance: 正常松手、指针取消、捕获丢失和拖动期间窗口变化均保留最后有效高度，只有非拖动窗口变化恢复已保存偏好（covers: S7, S9; depends: T7）
- [x] T13: 重新交付拖动修复 — acceptance: 定向回归、完整检查、构建、独立审查及 VSIX 哈希复核通过（covers: S6, S9; depends: T12）
