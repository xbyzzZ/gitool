...
- 0.2.10 的原生 TreeView 能跟随主题，但破坏了用户需要的行内展开定位，因此 0.2.11 取消该试验视图，改为读取已安装扩展公开声明的文件图标主题贡献。
- 真实 Seti 主题的 `fontCharacter` 是 CSS 十六进制转义字符串而非私用区字符；必须保留合法转义，同时对实际 Unicode 字符逐码点编码。
- VS Code 文件图标主题是 JSONC，解析必须允许注释和尾逗号；父子主题资源分别相对各自主题文件解析，再合并定义与关联。
- 主题读取和提交详情都是异步操作；动态重载必须同时校验单调序号与 Webview 身份，拒绝过期成功、失败和已销毁视图结果。
- 首次打包把工作树内临时测试缓存纳入 VSIX；将依赖缓存移到系统临时目录后重新打包，最终清单不含测试缓存。
...
引用标签使用 VS Code 主题变量：HEAD 为主要强调，本地分支为分支标签，远程分支为低强调远程标签。长标题和长引用单行省略，完整内容保留在 title。提交行支持键盘焦点和行内展开；文件列表由 S8 定义的行内区域承载，点击文件继续打开历史 Diff。
