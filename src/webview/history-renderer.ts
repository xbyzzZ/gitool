...
export function renderCommitRowMarkup(
  ...
  const files = expanded
    ...
    : '';
  return `<article class="history-entry${expanded ? ' expanded' : ''}" data-hash="${commit.hash}" style="--graph-width:${String(options.graphWidth)}px">`
    + `<button class="history-commit-row" type="button" role="treeitem" aria-expanded="${String(expanded)}" title="${escapeHtml(title)}">`
