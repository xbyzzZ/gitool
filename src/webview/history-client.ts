...
const syncSummary = element('sync-summary');
const historyStatus = element('history-status');
const historyList = element('history-list');
const expandedCommits = new Set<string>();
const commitDetails = new Map<
  string,
  readonly CommitDetails['files'][number][]
>();
...
function render(model: RepositoryViewModel): void {
  currentModel = model;
  layout.setAttribute('aria-busy', model.history.kind === 'loading'
    ? 'true'
    : 'false');
  const sync = model.sync.kind === 'detached'
    ? '游离 HEAD'
    : model.sync.kind === 'no-upstream'
      ? '未设置上游'
      : `${model.sync.upstream} · ↑${String(model.sync.ahead)} ↓${String(model.sync.behind)}`;
  syncSummary.textContent = sync;
